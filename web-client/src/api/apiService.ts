...
import {
  GetAssetDetailssQueryResult,
  GetAssetsQueryResult,
  GetAssetTypesQueryResult,
} from "src/types/ApiTypes.ts";
...
export const getAssets = async ({ pageParam = 1, sortBy = "id-desc" }): Promise<GetAssetsQueryResult> => {
  // build the url to this format: http://....../v1/assets?page=1&sortBy=id-desc
...
export const getAssetDetails = async ({
  ...
  assetId: string;
}): Promise<GetAssetDetailssQueryResult> => {
  // build the url to this format: http://....../v1/assets/1
